...
class CommandInterpreter {
    ...
    private void loadStatement() {
        ...
        _database.put(fileName, Table.readTable(fileName));
    ...
    private void selectStatement() {
        Table selected = selectClause("toPrint");
        _input.next(";");
    ...
    Table tableDefinition(String name) {
        ...
        if (_input.nextIf("(")) {
            ...
            _input.next(")");
            created = new Table(name, columns);
        } else {
            _input.next("as");
            created = selectClause(name);
        }
        return created;
    }
    ...
    Table selectClause(String tableName) {
        ...
        if (_input.nextIf("as")) {
            ...
            titles.add(columns.get(0).name());
        }
        while (_input.nextIf(",")) {
            Column newColumn = columnSelector();
            columns.add(newColumn);
            if (_input.nextIf("as")) {
                String alias = name();
                titles.add(alias);
            } else {
                titles.add(newColumn.name());
            }
        }
        _input.next("from");
        ...
        Table result = new Table(tableName, titles);
        select(result, columns, iterators, conditions);
        return result;
    }
    ...
    Column columnSelector() {
        ...
        String colName = designation;
        if (_input.nextIf(".")) {
            tableName = _database.get(designation);
            colName = name();
        }
        return new Column(tableName, colName);
    }
    ...
    Condition condition(List<TableIterator> iterators) {
        ...
        String relations = _input.next(RELATION);
        String nextVal = _input.peek();
        Column col2;
        if (nextVal.contains("'")) {
        ...
        return new Condition(col1, relations, col2);
    }
    ...
    private void select(Table table, ArrayList<Column> columns,
        ...
        assert columns.size() == table.numColumns() : "column size mismatch";
            ...
            if (iterators.size() == 2) {
                ...
                while (iterators.get(1).hasRow()) {
                    if (Condition.test(conditions)) {
                        Row added = Row.make(columns);
                        table.add(added);
                    }
                    iterators.get(1).next();
                }
            } else if (Condition.test(conditions)) {
