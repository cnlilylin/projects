...
public class BPlusTree {
  ...
  /**
   *  The number of valid lead entries(records) in a tree, used to test if splitting 
   *  changed leaf entry number
   * */
   ...
   */
    ...
    public BPlusIterator(LeafNode leaf) {
      //TODO: Implement Me!
        ...
        this.isScan=true;
        this.lookupKey=null;
        this.currLeafIter=this.selectIterator(this.currLeaf);
    }
   ...
   */
    ...
    /**
     * Selects type of record iterator for the current BPlusIterator.
     */
    private Iterator<RecordID> selectIterator(LeafNode node) {
        if (this.lookupKey==null) {
            return node.scan();
        } else {
            if (isScan) {
                return node.scanFrom(this.lookupKey);
            } else {
                return node.scanForKey(this.lookupKey);
            }
        }
    }
    ...
    public boolean hasNext() {
      //TODO: Implement Me!
        if (this.currLeafIter.hasNext()) {
            ...
            LeafNode test=this.currLeaf;
            while (test.getNextLeaf()!=-1) {
                test=(LeafNode)BPlusNode.getBPlusNode(BPlusTree.this, test.getNextLeaf());
                Iterator<RecordID> testIter=selectIterator(test);
                if (testIter.hasNext()) {
                    return true;
                }
            }
