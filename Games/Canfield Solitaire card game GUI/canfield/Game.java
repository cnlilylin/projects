...
class Game {
    ...
    /** List for the game states during a game. */
    private ArrayList<Game> _gameStates;
    ...
    void copyFrom(Game game0) {
        ...
        this._gameStates = game0._gameStates;
        for (int i = 0; i < TABLEAU_SIZE; i += 1) {
    ...
    void deal() {
        ...
        _stock.move(deck);
        _waste.clear();
        _gameStates = new ArrayList<Game>();
        _gameStates.add(new Game(this));
    }
    ...
    /** Undo method: return the last copy of game state.
     * returns current game state if no moves have been made /
     */
    void undo() {
        if (_gameStates.size() <= 1) {
            return;
        }
        copyFrom(_gameStates.get(_gameStates.size() - 1));
